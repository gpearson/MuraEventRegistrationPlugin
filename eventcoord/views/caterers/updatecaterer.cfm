...
<cfelseif isDefined("URL.PerformAction")>
	<cfif not isNumeric(URL.RecNo)><cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=eventcoord:caterers" addtoken="false"></cfif>
	<cfswitch expression="#URL.PerformAction#">
		...
		<cfcase value="Delete">
			...
			<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=eventcoord:caterers&UserAction=DeleteFacility&SiteID=#rc.$.siteConfig('siteID')#&Successful=true" addtoken="false">
		</cfcase>
	</cfswitch>
</cfif>