...
<cfelseif isDefined("FORM.formSubmit") and isDefined("URL.EventID") and isDefined("FORM.PerformAction")>
	...
	<cfswitch expression="#FORM.PerformAction#">
		<cfcase value="Step2">
			...
			<cfif FORM.GroupPrice_Available CONTAINS 1>
				...
				<cftry>
					<cfquery name="updateEvent" Datasource="#$.globalConfig('datasource')#" username="#$.globalConfig('dbusername')#" password="#$.globalConfig('dbpassword')#">
						...
						Set GroupPrice_Available = <cfqueryparam value="1" cfsqltype="cf_sql_bit">,
							GroupPrice_Requirements = <cfqueryparam cfsqltype="cf_sql_varchar" value="#FORM.GroupPrice_Requirements#">,
							GroupPrice_MemberCost = <cfqueryparam value="#FORM.GroupPrice_MemberCost#" cfsqltype="cf_sql_double">,
							GroupPrice_NonMemberCost = <cfqueryparam value="#FORM.GroupPrice_NonMemberCost#" cfsqltype="cf_sql_double">,
				...
				<cfif LEN(cgi.path_info)>
					<cfset newurl = #cgi.script_name# & #cgi.path_info# & "?" & #Session.PluginFramework.Action# & "=eventcoordinator:events.updateevent&UserAction=EventGroupPrice&EventID=#URL.EventID#&Successful=True" >
				<cfelse>
					<cfset newurl = #cgi.script_name# & "?" & #Session.PluginFramework.Action# & "=eventcoordinator:events.updateevent&UserAction=EventGroupPrice&EventID=#URL.EventID#&Successful=True" >
				</cfif>
				<cflocation url="#variables.newurl#" addtoken="false">			
			<cfelse>
				<cftry>
					<cfquery name="updateEvent" Datasource="#$.globalConfig('datasource')#" username="#$.globalConfig('dbusername')#" password="#$.globalConfig('dbpassword')#">
						Update p_EventRegistration_Events
						Set GroupPrice_Available = <cfqueryparam value="0" cfsqltype="cf_sql_bit">,
							lastUpdated = <cfqueryparam cfsqltype="cf_sql_timestamp" value="#Now()#">,
							lastUpdateBy = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Session.Mura.Fname# #Session.Mura.LName#">,
							lastUpdateByID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Session.Mura.UserID#">
						Where TContent_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#URL.EventID#">
