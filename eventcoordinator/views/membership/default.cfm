...
<cfoutput>	    
	...
	<div class="panel panel-default">
		<div class="panel-body">
			<fieldset>
				<legend><h2>Available Member Organizations</h2></legend>
			</fieldset>
			<cfif isDefined("URL.UserAction")>
				<cfswitch expression="#URL.UserAction#">
					<cfcase value="MemberFacilityUpdated">
								...
								<div class="modal-dialog">
									<div class="modal-content">
										<div class="modal-header">
											...
											<h3>Member Organization Updated</h3>
										</div>
										<div class="modal-body">
											<p class="alert alert-success">You have successfully updated a member organization in the database.</p>
										</div>
					...
					<cfcase value="MemberFacilityAdded">
						<cfif URL.Successful EQ "true">
							<div id="modelWindowDialog" class="modal fade">
								<div class="modal-dialog">
									<div class="modal-content">
										<div class="modal-header">
											<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times-circle"></i></button>
											<h3>Member Organization Created</h3>
										</div>
										<div class="modal-body">
											<p class="alert alert-success">You have successfully created a new membership organization in the database.</p>
										</div>
										<div class="modal-footer">
											<button class="btn btn-default" data-dismiss="modal" aria-hidden="true">Close</button>
										</div>
	...
	<script type="text/javascript">
		$(document).ready(function () {
			var selectedRow = 0;
			$("##jqGrid").jqGrid({
				url: "/plugins/#Variables.Framework.Package#/eventcoordinator/controllers/membership.cfc?method=getAllMembers",
				// we set the changes to be made at client side using predefined word clientArray
				datatype: "json",
				colNames: ["Rec No", "Member Name","Address","City","State","Voice Number","Last Updated","Active"],
				colModel: [
					{ label: 'Rec ##', name: 'TContent_ID', width: 75, key: true, hidden: true, editable: false },
					{ label: 'Member Name', name: 'OrganizationName', width: 75, editable: false },
			...
			$('##jqGrid').navButtonAdd('##jqGridPager',
				{
					...
					onClickButton: function(id) {
						var urlToGo = "http://" + window.location.hostname + "#cgi.script_name#" + "#cgi.path_info#?#rc.pc.getPackage()#action=eventcoordinator:membership.addmember";
			...
			$('##jqGrid').navButtonAdd('##jqGridPager',
				{
					...
					onClickButton: function(id) {
						if (selectedRow == 0) {
							...
							var urlToGo = "http://" + window.location.hostname + "#cgi.script_name#" + "#cgi.path_info#?#rc.pc.getPackage()#action=eventcoordinator:membership.editmember&MemberID="+ RowIDValue;
							window.open(urlToGo,"_self");
						}
						},
					position: "last"
